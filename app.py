...
def main():
    ...
    with st.sidebar:
        ...
        length_choice = st.selectbox(
            "Length", ["Short (~180–250 words)", "Medium (~250–350 words)", "Long (~350–500 words)"], index=1
        )
    ...
    st.write(
        "Upload your **CV** and any **additional files** (job description, summary, notes). Supports PDF, DOCX, TXT."
    )
    col1, col2 = st.columns(2)

    with col1:
        cv_file = st.file_uploader(
            "CV (PDF/DOCX preferred)", type=["pdf", "docx", "txt"], accept_multiple_files=False, key="cv"
        )
    with col2:
        additional_docs = st.file_uploader(
            "Additional documents (optional) – multiple allowed",
            type=["pdf", "docx", "txt"],
    ...
    with col_left:
        ...
        generate_btn = st.button(
            "🚀 Generate Cover Letter", type="primary", use_container_width=True, disabled=is_generate_disabled
        )
    with col_right:
    ...
    if generate_btn:
        ...
        st.download_button(
            label="⬇️ Download TXT", data=generated_text, file_name="cover_letter.txt", mime="application/txt"
        )
